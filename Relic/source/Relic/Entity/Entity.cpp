...
namespace Relic
{
    ...
    void Entity::SetFillColor(uint8_t r, uint8_t g, uint8_t b, uint8_t a)
    {
        uint32_t color;
        uint8_t bytes[4] { a, b, g, r };
        std::memcpy(&color, bytes, sizeof(uint32_t)); 
        GetComponent<Shape>().shape.SetFillColor(color); 
    }
}
